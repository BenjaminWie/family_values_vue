<template>
    <div class="value-container">
        <!-- Main Value Display -->
        <div class="value" @mouseover="hoverValue" @mouseleave="resetValue" @click="toggleExpand"
        ...
        <div v-if="expandedOnce">
            <div v-for="(item, index) in floatingItems" :key="item.title" :style="floatingStyles[index]"
                class="floating-item" @click="expandFloatingItem(item)">
                <h3 :class="item.type === 'narrative' ? 'narrative-title' : 'story-title'">{{ item.title }}</h3>
...
export default {
    ...
    methods: {
        ...
        initializeFloatingItems() {
            this.floatingItems = [...this.value.narratives.map(narrative => ({ ...narrative, type: 'narrative' })),
